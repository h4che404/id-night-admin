...
import { findAccess } from "@/lib/data";
      ...
      <div className="grid gap-6 xl:grid-cols-[1.4fr_1fr]">
        <Surface className="p-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm text-slate-400">{access.person}</p>
              <h2 className="mt-2 text-2xl font-semibold text-white">{access.venue} · {access.gate}</h2>
            </div>
            <Badge label={access.result} tone={toneForLabel(access.result)} />
          </div>
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            <ValuePair label="Fecha y hora" value={formatDate(access.timestamp)} />
            <ValuePair label="Operador" value={access.operator} />
            <ValuePair label="Motivo" value={access.reason} />
            <ValuePair label="Alerta" value={access.alert} />
          </div>
        </Surface>
